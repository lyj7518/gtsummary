...
**add_p()** 함수를 사용하여 `tbl_summary`에 의해 생성된 테이블에 p-value를 추가할 수 있다. 이때 test 인자에 수행할 통계 테스트를 지정할 수 있다. 또한 사용자 정의 테스트 함수는 모든 변수 또는 일부 변수에 대해 추가할 수 있다.
(기본값: 연속형 변수- "kruskal.test", 범주형 변수- 행의 수가 5 이상인 경우 "chisq.test", 5 미만인 경우 "fisher.test" )
형식은 ~ 
...
다양한 **add_()**함수를 이용하여 앞에서 만든 table에 여러 통계량을 추가해보자.
