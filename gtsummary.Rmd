...
`tbl_strata` 함수를 이용하여 두번에 걸쳐 그룹으로 묶을 수 있다. 형식은 **tbl_strata()**를 사용한다. 여러 계층으로 그룹을 묶을 수 있다.
...
dt2 %>% 
  tbl_strata(
    ...
    .tbl_fun = 
      ~ .x %>% 
      tbl_summary(by=EXMD_BZ_YYYY, missing="no") %>% 
      add_n() %>% 
      add_p(),
    .header = "**{strata}**, N={n}"
  )
...
**add_p()** 함수를 사용하여 `tbl_summary`에 의해 생성된 테이블에 p-value를 추가할 수 있다. 이때 test 인자에 "t.test", "wilcox.test", "chisq.test", "fisher.test" 와 같은 수행할 통계 테스트를 지정할 수 있다. **add_p(col ~ ".test")** 형식을 사용하며 모든 변수 또는 일부 변수에 대해 적용할 수 있다.

(default test: <u>연속형 변수</u> - 2그룹 **wilcox.test**, 3그룹 이상 **kruskal.test**
...
연속형 변수의 통계량은 여러 줄로 요약될 수 있다. 여러 줄에 걸쳐 연속형 변수의 요약값을 출력할 때에는 변수 type을 **continuous2**로 지정해주어야 한다.
...
3. **add_()**와 다양한 함수를 통해 테이블에 통계량을 추가하고 디자인을 설정할 수 있다.
